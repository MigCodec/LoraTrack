...
{
    /** @var array<string, Collection<int, AssetDeviceAssignment>> */
    private array $assignmentHistory = [];

    /** @var array<string, MerakiFloorPlanMapping|null> */
    private array $floorPlanMappings = [];
    ...
    {
        $history = $this->assignmentHistory[$deviceId] ??= AssetDeviceAssignment::query()
            ->with('asset')
            ->where('device_id', $deviceId)
            ->latest('started_at')
            ->get();

        return $history->first(fn (AssetDeviceAssignment $assignment): bool =>
            $assignment->started_at->lte($seenAt)
            && ($assignment->ended_at === null || $assignment->ended_at->gte($seenAt))
        );
    }
    ...
    private function storePosition(
        ...
        if ($externalFloorId !== '') {
            $mappingKey = $event->connector_id.'|'.$externalFloorId;
            if (array_key_exists($mappingKey, $this->floorPlanMappings)) {
                $mapping = $this->floorPlanMappings[$mappingKey];
            } else {
                $mapping = MerakiFloorPlanMapping::query()
                    ...
                    ->first();
                $this->floorPlanMappings[$mappingKey] = $mapping;
            }
        }
