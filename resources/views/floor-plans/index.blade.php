            ...
            <aside class="space-y-6">
                @if(auth()->user()->hasPermission('plans.manage') && $selectedPlan->drawablePath())
                    ...
                    <form id="anchor-form" method="POST" action="{{ route('installations.store', $selectedPlan) }}" class="panel p-5" hidden>
                        ...
                        <div class="mt-5 space-y-4">
                            <label class="field-label">Dispositivo<select class="field-input" name="device_id" required>@foreach($devices->whereIn('type', ['beacon', 'scanner']) as $device)<option value="{{ $device->id }}">{{ $device->name }} · {{ $device->identifier }}</option>@endforeach</select></label>
                            ...
                            <button id="anchor-mode" class="btn-secondary w-full" type="button" @disabled($devices->whereIn('type', ['beacon', 'scanner'])->isEmpty())>Seleccionar otro punto</button>
