<form id="anchor-form" method="POST" action="{{ route('installations.store', $selectedPlan) }}" class="mt-4 space-y-4 fixed-reference-form">
    @csrf
    <p class="text-xs leading-relaxed text-slate-500">Agrega infraestructura fija con coordenadas conocidas. Completa el flujo de arriba hacia abajo para no mezclar beacons fijos con AP/scanners fijos.</p>

    <section class="fixed-reference-step">
        <header><span>1</span><div><h3>Tipo</h3><p>Define que topologia usara este punto.</p></div></header>
    ...
    <section class="fixed-reference-step">
        <header><span>2</span><div><h3>Posicion</h3><p>Marca el punto conocido o escribe sus coordenadas reales.</p></div></header>
        ...
        @if($selectedPlan->drawablePath())
            <div class="fixed-reference-position-actions">
    ...
    <section class="fixed-reference-step">
        <header><span>3</span><div><h3>Dispositivo</h3><p>Busca uno existente o crea uno con su MAC.</p></div></header>
    ...
    <section class="fixed-reference-step">
        <header><span>4</span><div><h3>Calibracion</h3><p>Ajustes usados por los calculos RSSI.</p></div></header>
