        ...
        <form id="asset-form" class="panel space-y-4 p-6" method="POST" enctype="multipart/form-data" action="{{ $asset->exists ? route('assets.update', $asset) : route('assets.store') }}">
            ...
            <label class="field-label">Tipo
                <select class="field-input" name="mobility">
                    <option value="mobile" @selected(old('mobility', $asset->mobility) === 'mobile')>Móvil</option>
                    <option value="static" @selected(old('mobility', $asset->mobility) === 'static')>Estático</option>
                </select>
            </label>
            <label class="field-label">Ubicación asignada
            ...
            @unless($asset->exists)
                <label class="field-label" data-mobile-tracker-field>Tracker LoRaWAN inicial (opcional)
                    <select class="field-input" name="tracker_device_id">
                        ...
                        @foreach($reportedTrackers as $tracker)
                            <option value="{{ $tracker->id }}" @selected(old('tracker_device_id') === $tracker->id)>{{ $tracker->name }} · {{ $tracker->model ?: 'Sin modelo' }} · {{ $tracker->identifier }}</option>
                        @endforeach
                    </select>
                ...
                <label class="field-label" data-static-beacon-field>Beacon BLE inicial (opcional)
                    <select class="field-input" name="static_beacon_device_id">
                        <option value="">Asignar después</option>
                        @foreach($availableBeacons as $beacon)
                            <option value="{{ $beacon->id }}" @selected(old('static_beacon_device_id') === $beacon->id)>{{ $beacon->name }} · {{ $beacon->identifier }}</option>
                        @endforeach
                    </select>
                    <span class="mt-1 block text-xs font-normal text-slate-400">Para activos estáticos, selecciona el beacon BLE unido físicamente al activo. Solo aparecen beacons activos y sin asignación.</span>
                </label>
            @endunless

            <label class="field-label">Fotografía del activo (opcional)
                <input class="field-input" type="file" name="photo" accept="image/jpeg,image/png,image/webp">
                <span class="mt-1 block text-xs font-normal text-slate-400">JPG, PNG o WEBP; máximo 8 MB. Se almacena de forma privada.</span>
            </label>

            @if($asset->exists && $asset->photo_path)
                <div class="rounded-xl border border-slate-200 p-3">
                    <img class="max-h-52 w-full rounded-lg object-contain" src="{{ route('assets.photo', $asset) }}" alt="Fotografía de {{ $asset->name }}">
                    <label class="mt-3 flex items-center gap-2 text-sm"><input type="checkbox" name="remove_photo" value="1"> Eliminar fotografía actual</label>
                </div>
            @endif
            ...
            <section class="panel p-6">
                    ...
                    <div class="my-4 rounded-xl bg-emerald-50 p-4 text-sm">
                        ...
                        @if($asset->mobility === 'mobile' && $current->tracking_strategy === 'fixed_beacons_mobile_tracker')
                            <form class="mt-3" method="POST" action="{{ route('assets.position.refresh', $asset) }}">
                                @csrf
                                <button class="btn-secondary" type="submit">Recalcular ubicación</button>
                            </form>
                        @endif
                        <form class="mt-2" method="POST" action="{{ route('asset-assignments.destroy', $current) }}">
                            @csrf
                            @method('DELETE')
                            <button class="text-red-600">Finalizar asignación</button>
                        </form>
                    </div>
                @endif
                ...
                @if($asset->mobility === 'mobile')
                    <form method="POST" action="{{ route('asset-assignments.store', $asset) }}" class="mt-5 space-y-4">
                        ...
                        <label class="field-label">Tracker registrado
                            <select class="field-input" name="device_id">
                                ...
                                @foreach($reportedTrackers as $tracker)
                                    <option value="{{ $tracker->id }}">{{ $tracker->name }} · {{ $tracker->model ?: 'Sin modelo' }} · {{ $tracker->identifier }}</option>
                                @endforeach
                            </select>
                        ...
                        @if($reportedTrackers->isEmpty())
                            <p class="text-xs text-amber-700">No hay trackers disponibles. Puedes escribir manualmente el DevEUI.</p>
                        @endif
                    ...
                    <form method="POST" action="{{ route('asset-assignments.store', $asset) }}" class="mt-8 space-y-4 border-t pt-5">
                        ...
                        <label class="field-label">Alternativa: beacon móvil
                            <select class="field-input" name="device_id" required>
                                @foreach($availableBeacons as $device)
                                    <option value="{{ $device->id }}">{{ $device->name }} · {{ $device->identifier }}</option>
                                @endforeach
                            </select>
                        </label>
                        <button class="btn-secondary" @disabled($availableBeacons->isEmpty())>Asignar beacon móvil</button>
                    </form>
                @else
                    <form method="POST" action="{{ route('asset-assignments.store', $asset) }}" class="mt-5 space-y-4">
                        @csrf
                        <input type="hidden" name="tracking_strategy" value="mobile_beacon_fixed_scanners">
                        <label class="field-label">Beacon del activo estático
                            <select class="field-input" name="device_id" required>
                                @foreach($availableBeacons as $device)
                                    <option value="{{ $device->id }}">{{ $device->name }} · {{ $device->identifier }}</option>
                                @endforeach
                        ...
                        <button class="btn-primary" @disabled($availableBeacons->isEmpty())>Asignar beacon</button>
                    </form>
                @endif
