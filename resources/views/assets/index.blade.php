    ...
    <section class="panel">
        ...
        @else
            <div class="table-wrap"><table class="data-table"><thead><tr><th>Activo</th><th>SKU</th><th>Serial</th><th>Dispositivo</th><th>Ubicación</th><th>Estado</th></tr></thead><tbody>
                @foreach($assets as $asset)<tr><td><a class="font-semibold text-brand-primary" href="{{ route('assets.edit', $asset) }}">{{ $asset->name }}</a><br><span class="text-xs text-slate-400">{{ $asset->asset_tag }}</span></td><td>{{ $asset->sku?->code ?? '—' }}</td><td>{{ $asset->serial_number ?? '—' }}</td><td>{{ $asset->deviceAssignments->first()?->device?->name ?? 'Sin asignar' }}</td><td>{{ $asset->latestPosition?->zone?->name ?? $asset->location?->name ?? 'Desconocida' }}</td><td><span class="status-badge">{{ ucfirst($asset->status) }}</span></td></tr>@endforeach
            </tbody></table></div>
