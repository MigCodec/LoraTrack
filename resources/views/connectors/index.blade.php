...
@push('scripts')
    <script defer src="{{ asset('js/connectors-live.js') }}?v={{ filemtime(public_path('js/connectors-live.js')) }}"></script>
@endpush
@section('content')
    ...
    <section class="panel mt-10" data-connectors-live data-endpoint="{{ route('connectors.live-metrics') }}"><div class="panel-header"><div><h2 class="panel-title">Conectores configurados</h2><p class="panel-subtitle">Estado, actividad, eventos y acceso al log operacional</p></div><span class="connectors-live-status" data-live-status role="status" aria-live="polite"><i aria-hidden="true"></i><span>Actualización cada 5 s</span></span></div>
        @if($connectors->isEmpty())<div class="empty-state">Aún no hay conectores configurados.</div>@else
            <div class="table-wrap"><table class="data-table"><thead><tr><th>Nombre</th><th>Proveedor</th><th>Estado operacional</th><th>Eventos</th><th>Última actividad</th><th>Acciones</th></tr></thead><tbody>
                @foreach($connectors as $connector)<tr data-connector-id="{{ $connector->getRouteKey() }}">
                    ...
                    <td><span class="connector-live-metric"><strong data-live-metric="telemetry_events_count">{{ $connector->telemetry_events_count }}</strong></span><br><span class="text-xs text-slate-400"><span class="connector-live-metric"><span data-live-metric="processed_events_count">{{ $connector->processed_events_count }}</span></span> procesados · <span class="connector-live-metric"><span data-live-metric="failed_events_count">{{ $connector->failed_events_count }}</span></span> fallidos</span></td>
                    <td data-live-activity>{{ $connector->last_activity_at?->diffForHumans() ?? $connector->last_tested_at?->diffForHumans() ?? 'Sin actividad' }}</td>
